...
Standalone Slack client using Socket Mode to receive messages and forward to Gateway.
"""
...
from pyclaw.channels.slack.adapter import SlackAdapter, SlackConfig
from pyclaw.constants import DEFAULT_GATEWAY_URL

# Configure logging
LOG_DIR = os.path.expanduser("~/.pyclaw")
os.makedirs(LOG_DIR, exist_ok=True)
LOG_FILE = os.path.join(LOG_DIR, "slack_client.log")
    ...
    async def _handle_message(self, message: Any) -> None:
        ...
        try:
            ...
            if assistant_message:
                # Send reply
                target = message.raw_data.get("channel", message.peer_id)
                reply_to = message.raw_data.get("thread_ts") or message.raw_data.get("ts")
...
def load_config() -> SlackConfig:
    ...
    return SlackConfig(
        bot_token=os.environ.get("SLACK_BOT_TOKEN", slack_config.get("bot_token", "")),
        app_token=os.environ.get("SLACK_APP_TOKEN", slack_config.get("app_token", "")),
        signing_secret=os.environ.get("SLACK_SIGNING_SECRET", slack_config.get("signing_secret", "")),
        gateway_url=slack_config.get("gateway_url", DEFAULT_GATEWAY_URL),
        gateway_token=slack_config.get("gateway_token")
    )
...
async def main() -> None:
    ...
    if not config.bot_token or not config.app_token:
        logger.error("Slack configuration incomplete, please set SLACK_BOT_TOKEN and SLACK_APP_TOKEN")
        sys.exit(1)
